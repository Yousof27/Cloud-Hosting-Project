    ...
    <div className="flex items-center justify-center !mt-2 !mb-10">
      ...
      {pagesArray.map((page) => (
        <Link
          ...
          } border border-gray-700 text-gray-700 !py-1 !px-3 font-bold text-xl cursor-pointer hover:bg-gray-400 transition`}
        >
          {page}
        </Link>
      ))}
      {pageNumber !== pages && pages !== 0 && (
        <Link
          href={`${route}?pageNumber=${pageNumber + 1}`}
          className="border border-gray-700 text-gray-700 !py-1 !px-3 font-bold text-xl cursor-pointer hover:bg-gray-200 transition"
        >
